...
public final class CodeScanner {//NOSONAR
    ...
    private String tagString = "CodeScanner";
    ...
    public CodeScanner(@NonNull final Context context, @NonNull final CodeScannerView view) {
        ...
        mPreviewCallback = new PreviewInnCallback();
        ...
        synchronized (mInitializeLock) {
            if (mInitialized && mPreviewActive && !mTouchFocusing) {
                try {
                    ...
                    Logger.LOGGER.throwing(tagString,1,  new Throwable(errorString),ignored.getMessage());
                }
            }
        }
    }
    ...
    private final class PreviewInnCallback implements Camera.PreviewCallback {
        @Override
    ...
    private final class InitializationThread extends Thread {
        ...
        private void initialize() {
            ...
            final Point imageSize = getImageSize(parameters, portrait);
            ...
            final Point previewSize = getPreviewSize(portrait, imageHeight, imageWidth);
        ...
        private Point getPreviewSize(boolean portrait, int imageHeight, int imageWidth) {
            return Utils.getPreviewSize(portrait ? imageHeight : imageWidth,
                    portrait ? imageWidth : imageHeight, mWidth, mHeight);
        }

        private Point getImageSize(Parameters parameters, boolean portrait) {
            return Utils.findSuitableImageSize(parameters, portrait ? mHeight : mWidth,
