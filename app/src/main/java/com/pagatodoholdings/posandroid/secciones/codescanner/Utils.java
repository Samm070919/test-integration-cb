package com.pagatodoholdings.posandroid.secciones.codescanner;
...
final class Utils {//NOSONAR
    ...
    public static void disableAutoFocus(@NonNull final Parameters parameters) {
        ...
        if (focusModes.contains(Parameters.FOCUS_MODE_AUTO) && !Parameters.FOCUS_MODE_AUTO.equals(focusMode)) {
            parameters.setFocusMode(Parameters.FOCUS_MODE_AUTO);
        }
    }
    ...
    public static byte[] rotateYuv(@NonNull final byte[] source, final int width, final int height,
        ...
        for (int j = 0; j < height; j++) {
            for (int i = 0; i < width; i++) {
                ...
                final int vIn = uIn + 1;
                final int wOut = getwOut(swap, height, width);
                final int hOut = gethOut(swap, height, width);
                final int iSwapped = iSwapped(swap, i, j);
                final int jSwapped = jSwapped(swap, i, j);
                final int iOut = getiOut(flipX, wOut, iSwapped);
                final int jOut = getjOut(flipY, hOut, jSwapped);
    ...
    private static int getjOut(boolean flipY, int hOut, int jSwapped) {
        return flipY ? hOut - jSwapped - 1 : jSwapped;
    }

    private static int getiOut(boolean flipX, int wOut, int iSwapped) {
        return flipX ? wOut - iSwapped - 1 : iSwapped;
    }

    private static int jSwapped(boolean swap, int i, int j) {
        return swap ? i : j;
    }

    private static int iSwapped(boolean swap, int i, int j) {
        return swap ? j : i;
    }

    private static int gethOut(boolean swap, int height, int width) {
        return swap ? width : height;
    }

    private static int getwOut(boolean swap, int height, int width) {
    ...
    private static final class FpsRangeComparator implements Comparator<int[]> {
        ...
        public int compare(final int[] a, final int[] b) {
            int comparison = Integer.compare(b[PREVIEW_FPS_MAX_INDEX],
                    a[PREVIEW_FPS_MAX_INDEX]);
            if (comparison == 0) {
                comparison = Integer.compare(b[PREVIEW_FPS_MIN_INDEX],
                        a[PREVIEW_FPS_MIN_INDEX]);
            }
            return comparison;
        }
    }
